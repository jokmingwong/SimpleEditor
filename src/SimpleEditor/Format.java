 ...
 * Format.java
 * æ ¼å¼åŒ–ä»£ç 
 * è¯·ç›´æŽ¥è°ƒç”¨Format.format(textArea);
 * @author LiNan
 **/
...
public class Format {
	...
	public static String changeStrToUUid(String string,String type){
	    ...
	    int indexHome = -1; //ï¿½ï¿½Ê¼ï¿½ï¿½È¡ï¿½Â±ï¿½
...
/**
 * ¸ñÊ½»¯´úÂë
 * ÇëÖ±½Óµ÷ÓÃFormat.format(textArea);
 * @author LiNan
 **/
public class Format {
	...
	public static Map<String,String> mapZY = new HashMap<String,String>();
	/**
	 * Ñ­»·Ìæ»»Ö¸¶¨×Ö·ûÎªËæ»úuuid  ²¢½«uui´æÈëÈ«¾Ömap:mapZY   
	 * @param string   ×Ö·û´®
	 * @param type    Ö¸¶¨×Ö·û
	 **/
	public static String changeStrToUUid(String string,String type){
	    ...
	    boolean bool = false;
	    StringBuilder sb = new StringBuilder();
	    int indexHome = -1; //¿ªÊ¼½ØÈ¡ÏÂ±ê
	    while(slashMatcher.find()) {
	       int indexEnd = slashMatcher.start();
	       String tmp = string.substring(indexHome+1,indexEnd); //»ñÈ¡"ºÅÇ°ÃæµÄÊý¾Ý
	       if(indexHome == -1 ||bool == false){
	    	   sb.append(tmp);
	    	   bool = true;
	    	   indexHome = indexEnd;
